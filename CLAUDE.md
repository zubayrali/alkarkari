...
| `pnpm run stage <x>` | Copy `locales/<x>/` into the live (gitignored) `content/` + `public/`. Must be `pnpm run stage` — bare `pnpm stage` hits pnpm's built-in stage command (`ERR_PNPM_STAGE_SUBCOMMAND_REQUIRED`) |
...
**Custom sidenote syntax** (`{{note}}` and `{{sidenotes[label]: content}}`)
- `lib/remark-sidenote-syntax.ts` `transformSidenoteSyntax()` — runs at generation time (before MDX parsing; called from `scripts/generate.ts`) to convert both forms into GFM footnotes: bare `{{note}}` (kufrCleaner-style unlabeled marginalia, marker glued to the preceding word) → `[^_sn_N]`, labeled `{{sidenotes[label]: content}}` → `label[^_sn_N]`. Code fences, inline code, and YAML frontmatter are skipped; multi-line note bodies fold to one line. Must happen before MDX sees the `{{` (acorn treats it as a JSX expression). Citations inside the content (e.g. `[@key]`) resolve normally since the transform produces standard GFM footnotes that flow through the `rehypeCitations` → `rehypeSidenotes` pipeline. Tests: `tests/sidenote-syntax.test.ts`.
- `lib/remark-sidenote-syntax.ts` — the single implementation: exports the pure `transformSidenoteSyntax()` plus a `remarkSidenoteSyntax()` plugin wrapper for non-generation contexts
...
- `components/graph-view.tsx` — canvas renderer: degree-sized nodes, zoom-faded labels, focus-on-hover dimming (tweened), visited tint (localStorage `graph-visited`), radial layout on the global variant, zoom-to-fit/fullscreen controls. Auto-fit runs **once per dataset** — after that, pan/zoom belongs to the user (no refit on engine stop or resize); the fit button re-fits manually. Chrome (frame, controls, depth select, tooltip, legend) is styled by `app/graph-ledger.css` (Night Instruments pass); the renderer reads `--graph-tag-color` (muraqqaʿa patch-6/ray-6, replacing teal) and `--font-mono` at runtime via `readThemeColors`, so palette changes belong in that stylesheet, not the component.
- `components/local-graph.tsx` — client local graph ("Connections"), mounted via the `Backlinks` component's `graph` prop in the page body; depth selector re-slices the full graph client-side. Fumadocs' TOC surfaces are disabled everywhere (`tableOfContent` and `tableOfContentPopover` both `enabled: false` in the catch-all route); Night Threshold pages carry their own "Contents" dropdown (`components/night-contents.tsx`, a native `<details>`) plus the reading time in the black utility strip, and chromeless pages have no TOC at all.
...
- `components/sidenotes.tsx` + `app/sidenotes.css` — client sidenote engine, two placements: (1) true margin notes when ≥10rem of free margin exists beside the article (measured against `#nd-sidebar`; with no TOC column the right boundary is the viewport edge) — per-note width adapts 8–16rem to the available margin; notes alternate sides when both margins have room; (2) floating-ui click-popover otherwise. **Styling** (see `DESIGN.md`): plain Tufte marginalia — no box/border/fill, smaller serif (Spectral) gloss, a quiet superscript reference in the running ink (no gold), a hanging lead figure (CSS `::before` from `data-sidenote-number`), and a faint ink wash (`currentColor` 6%) as the only hover decoration. **Deep-linking**: each marker is a keyboard-reachable link; activating it (or a margin note) writes `#sidenote-N` to the URL via `history.replaceState` (shareable, no history entry) and flashes a soft ink wash that fades (`.sidenote-arrive`). On load / `hashchange`, a `#sidenote-N` hash smooth-scrolls to the note and washes it (opening its popover after the scroll settles if in popover mode). Margin notes are `z-index: 5`, below the Night Threshold header (z-6) so the open Contents dropdown covers them. **Margin balance** comes from an `app/global.css` override on `#nd-docs-layout`: `--fd-layout-width: 120rem` (past fumadocs' 1400px cap). A second override sets `--fd-sidebar-width` (272px, 300px ≥100rem) to give the explorer comfortable, screen-responsive room. The layout-width override is load-bearing: without it, on viewports wider than the cap the surplus width pools in the trailing (ex-TOC) grid column as dead space beside the right notes while the left notes stay pinned to the sidebar, so the two margins measure unequal (left notes narrower). Widening collapses the dead column, re-centering the article with symmetric margins
...
**Home page & site theme** ("Primordial Light" design system — see `DESIGN.md`, ADR-0014)
- `app/(home)/page.tsx` — shell home page (ADR-0003), a dark→light scroll journey: night hero (Light Verse + interactive prism), `.kk-journey-dark` zone (intention band, featured "Start here" card (`featured: true` frontmatter, dictionary fallback), context gallery, guide section), dawn gradient, light zone (pathways grid, recent notes, key terms), zāwiya footer
...
- `components/home/` — `hero.tsx`, `sections.tsx`, `footer.tsx` (server; ALL copy comes from `lib/locale.ts` `home` strings — never hardcode homepage prose), `patchwork.tsx` (`PatchworkBand` — the cloak-hem colour band) + `reveal.tsx` (`ScrollReveal`/`ScrollRevealItem`) + `motion-primitives.tsx` (`FloatingParticles`/`Parallax`) (client animation primitives), `story/*` scroll-narrative pieces (`ScrollTimeline`, `LetterStaggerText`, `FoundationsGrid`, `ScrollProgressSpine`, `StickyScrollStory`), static-imported images (never `public/`, which generate wipes). Retired set-pieces (deleted, not just unmounted): the prism/`LightRefraction` refraction fan and the `MosaicStar` khatim mosaic. Every real home component has a live labeled demo in `components/design/home-components.tsx` on `/design`
- `app/karkari-theme.css` — site-wide Fumadocs token overrides ("Primordial Light": night/pearl/lamp-gold + the 12-colour muraqqaʿa spectrum and its `--kk-ray-*` luminous variants, plus motif utilities — stitch/niche/veil/thread/halo/journey); imported last in `global.css`. The `.kk-journey-dark` zone remaps fd tokens locally so components stay on fd tokens. Gold text always via `--kk-gold-ink` (AA per mode); `--kk-gold` is decorative-only in light mode.
...
- Sidenote syntax (`{{note}}` / `{{sidenotes[label]: content}}`) is transformed at generation time, not at MDX compile time. MDX's acorn parser treats `{{` as a JSX expression and crashes if the syntax reaches it. `transformSidenoteSyntax()` (`lib/remark-sidenote-syntax.ts`) must run before any MDX processing. **Any double braces in vault prose become a sidenote** (code fences/inline code/frontmatter excepted) — escape or rephrase literal `{{…}}`. Hand-maintained `content/` files cannot use this syntax — use standard GFM footnotes (`[^n]`) instead.
...
contract), `sidenotes` (marginalia authoring syntax + rendering contract),
`local-setup` (fresh-machine onboarding), `visual-qa` (human eyeball
checklist — the agent has no browser tooling).
