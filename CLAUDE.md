...
- `components/sidenotes.tsx` + `app/sidenotes.css` — client sidenote engine, three placements in priority order: (1) true margin notes when free margin space exists (measured against `#nd-sidebar`/`#nd-toc` — this is the reader-mode path, where the TOC is hidden); (2) stacked cards in the `#sidenote-rail` below the TOC (`tableOfContent.footer` in the catch-all route renders the container, the engine fills it with styled clones of `.sidenote-content`; label hover/click cross-highlights and scrolls the card into view); (3) floating-ui click-popover on `< xl` viewports where the TOC column doesn't exist. So sidenotes are always visible on desktop, in or out of reader mode, sidebar collapsed or not
